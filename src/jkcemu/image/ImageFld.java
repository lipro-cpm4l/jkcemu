...
public class ImageFld extends Component implements Printable
{
  ...
  public ImageFld( int defaultWidth, int defaultHeight )
  {
  ...
  public BufferedImage getNewPreviewImage()
  {
	...
	if( (wParent > 0F) && (hParent > 0F) && (wImg > 0F) && (hImg > 0F) ) {
	  ...
	  if( scale < 1F ) {
	    ...
	    previewImg = ImageUtil.createCompatibleImage( this.image, w, h );
	    if( previewImg != null ) {
  ...
  {
    return this.rotation.equals( Rotation.LEFT )
		|| this.rotation.equals( Rotation.RIGHT );
  }
