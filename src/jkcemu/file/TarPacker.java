			...
			implements FileVisitor<Path>
{
  ...
  public static void computeAndSetHeaderChecksum( byte[] headerBuf )
  {
    ...
    String text = String.format( "%06o", chkSum );
    writeASCII( headerBuf, 148, text );
    int len = text.length();
    if( len < 8 ) {
      headerBuf[ 148 + len ] = (byte) 0;
    }
  }
  ...
  public FileVisitResult visitFile( Path file, BasicFileAttributes attrs )
  {
	...
	if( namePath != null ) {
	  ...
	  if( fileName != null ) {
		...
		try {
		  in       = openInputFile( file.toFile(), null );
		  long len = in.getFileSize();
		  writeTarHeader(
			file,
			entryDir + fileName,
			'0',			// Typ: Datei
  ...
  private static void writeASCII( byte[] buf, int pos, String text )
  {
    if( text != null ) {
      int len = text.length();
      for( int i = 0; i < len; i++ ) {
			...
			boolean             defaultExec ) throws IOException
  {
	...
	try {
	  posixAttrs = Files.readAttributes(
					path,
    ...
    if( (owner == null) && this.ownerEnabled ) {
      try {
    ...
    // 148-155: Pruefsumme des Kopfblocks, wird speater gesetzt
    ...
    computeAndSetHeaderChecksum( headerBuf );
