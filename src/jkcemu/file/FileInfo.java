...
public class FileInfo
{
  ...
  {
    return analyzeFile( header, file, null );
  }
				...
				String fileName )
  {
	    ...
	    && EmuUtil.isHexChar( header[ 10 ] & 0xFF ) )
	{
      ...
      if( (fileFmt == null) && (upperFileName != null) ) {
	...
	if( (fileFmt == null) && upperFileName.endsWith( ".COM" )
	    && (fileLen > 0) )
	{
	  begAddr  = 0x0100;
	  fileFmt  = FileFormat.COM;
	  fileText = "COM-Datei";
	}
				...
				FileFormat fileFmt ) throws IOException
  {
	...
	if( fileFmt.equals( FileFormat.HEADERSAVE ) ) {
	  ...
	  if( fileBuf[ 16 ] >= 3 ) {
	    int startAddr = EmuUtil.getWord( fileBuf, 21 );
	    if( (startAddr > 0) && (startAddr < 0xFFFF) ) {
	      rv.setStartAddr( startAddr );
	    }
	  }
	  ...
	  rv = new LoadData(
		...
		fileFmt );
	} else if( fileFmt.equals( FileFormat.COM ) ) {
	  rv = new LoadData(
			fileBuf,
			0,
			fileBuf.length,
  ...
  {
    return FileUtil.readFile( file, false, 0x10000 * 13 );
  }
