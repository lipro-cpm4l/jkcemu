 ...
 *  B0 In:  RXF     - Daten koennen von VDIP gelesen werden (L-aktiv)
 *  B1 In:  TXE     - Daten koennen in VDIP geschrieben werden (L-aktiv)
 *  B2 Out: RD      - Datenbyte von VDIP lesen (L-aktiv)
 *  B3 Out: WR      - Datenbyte in VDIP schreiben (H-aktiv)
 *  B4 Out: DATAREQ - VDIP-Datenmodus anfordern (L-aktiv)
 *  B5 In:  DATAACK - VDIP ist im Datenmodus (L-aktiv)
 *  B6 Out: RS      - RESET (L-aktiv)
 *  B7 Out: PG      - Programmieren (L-aktiv)
 */
			...
			Z80PIOPortListener
{
  ...
  /*
   * Standard-Zeitstempel der VDIP Firmware:
   *   numerischer Wert: 0x31940000
   *   Laut VDIP-Dokumenation entspricht das dem 2004-12-04 00:00:00,
   *   aber laut Bit-Belegung entspricht das dem 2004-12-20 00:00:00.
   */
  ...
  private int                          bitMode;
  private volatile long                lastUpdFileMillis;
  private volatile Long                defaultFileMillis;
  ...
  private volatile boolean             fileNameMapperEnabled;
  ...
  private volatile VFATFileNameMapper  fileNameMapper;
  private Map<File,VFATFileNameMapper> dir2FileNameMapper;
  ...
  public VDIP( int vdipNum, Z80CPU cpu, String title )
  {
    this.vdipNum               = vdipNum;
    this.cpu                   = cpu;
    this.title                 = title;
    this.moduleTitle           = "Vinculum VDIP Modul";
    this.debugLevel            = 0;
    this.lastUpdFileMillis     = 0;
    this.defaultFileMillis     = null;
    ...
    this.fileNameMapperEnabled = false;
    this.fileWrite             = false;
    this.file                  = null;
    this.fileMillis            = null;
    this.raf                   = null;
    this.freeDiskSpace         = null;
    this.dir2FileNameMapper    = new HashMap<>();
  ...
  public void applySettings( Properties props )
  {
    String dirText = EmuUtil.getProperty(
			props,
			getPropNameDirectory() );
    setDirectory( dirText.isEmpty() ? null : new File( dirText ) );
    setFileNameMapperEnabled(
		EmuUtil.getBooleanProperty(
			props,
			getPropNameFileNameMapperEnabled(),
			false ) );
    setForceCurrentTimestamp(
		EmuUtil.getBooleanProperty(
			props,
			getPropNameForceCurrentTimestamp(),
			true ) );
    setForceLowerCaseFileNames(
		EmuUtil.getBooleanProperty(
			props,
			getPropNameForceLowerCaseFileNames(),
			false ) );
    setReadOnly(
		EmuUtil.getBooleanProperty(
			props,
  ...
  {
    return String.format(
		"jkcemu.usb.%d.file_name_mapper.enabled",
		this.vdipNum );
  }
  ...
  {
    return String.format(
		"jkcemu.usb.%d.force_current_timestamp",
		this.vdipNum );
  }
  ...
  public synchronized void setDirectory( File dirFile )
  {
    if( dirFile != null ) {
  ...
  public void setFileNameMapperEnabled( boolean state )
  {
    synchronized( this.lockObj ) {
      File curDir = this.curDir;
      if( state && (curDir != null) ) {
	VFATFileNameMapper fnMapper = this.dir2FileNameMapper.get( curDir );
	if( fnMapper == null ) {
	  fnMapper = new VFATFileNameMapper();
	  this.dir2FileNameMapper.put( curDir, fnMapper );
	}
	this.fileNameMapper = fnMapper;
      } else {
	this.fileNameMapper = null;
      }
      this.fileNameMapperEnabled = state;
    }
  }


  public void setForceCurrentTimestamp( boolean state )
  {
    this.forceCurTimestamp = state;
  }


  public void setForceLowerCaseFileNames( boolean state )
  {
  ...
  {
    if( (pio == this.pio) && (portInfo == Z80PIO.PortInfo.B) ) {
      switch( status ) {
	case INTERRUPT_ENABLED:
	  /*
	   * Wenn der Interrupt freigegeben wird,
	   * waehrend die Strobe-Leitung aktiv ist,
	   * wird ein Interrupt angemeldet.
	   * Da die PIO-Emulation den statischen Zustand der Strobe-Leitung
	   * nicht verwaltet und somit diesen Zustand nicht erkennen kann,
	   * wird dieses Verhalten ueber die Listener-Funktionalitaet
	   * realisiert.
	   */
	...
	case OUTPUT_CHANGED:
	  {
	    // Pull Down Widerstand an PIO B3 -> 0xF7
	    int     tmpValue = this.pio.fetchOutValuePortB( 0xFF );
		...
		if( wrState ) {
		  ...
		  if( this.writeEnabled ) {
		    ...
		    int b = this.pio.fetchOutValuePortA( 0xFF );
		  ...
		  /*
		   * Strobe wird nur ausgeloest,
		   * wenn das Lesesignal inaktiv ist.
		   */
		  if( !rdState ) {
  ...
  public void run()
  {
	...
	if( this.ioTaskEnabled ) {
	  ...
	  File    oldRootDir     = null;
	  synchronized( this ) {
	    newRootDir          = this.newRootDir;
	    oldRootDir          = this.rootDir;
	    rootDirChanged      = this.rootDirChanged;
	    this.rootDirChanged = false;
	    this.newRootDir     = null;
	  }
	  if( rootDirChanged ) {
	    if( (newRootDir != null) && (oldRootDir != null) ) {
	      if( newRootDir.equals( oldRootDir ) ) {
		rootDirChanged = false;
	      }
	    } else if( (newRootDir == null) && (oldRootDir == null) ) {
	      rootDirChanged = false;
	    }
	  }
	  if( rootDirChanged ) {
  ...
  private void execChangeDir() throws VdipException
  {
    ...
    if( dirName == null ) {
      dirName = nextArgFileName( true, true );
    }
    synchronized( this.lockObj ) {
      File curDir  = this.curDir;
	...
	if( dirName.equals( "\\" ) ) {
      ...
      if( !done ) {
	File subDir = findFile( dirName, true );
	if( subDir == null ) {
	  throwCommandFailed();
	}
  ...
  {
    String dirName = nextArgFileName( false, true );
    synchronized( this.lockObj ) {
      checkDisk();
      if( this.raf != null ) {
	throwFileOpen();
      }
      File file = findFile( dirName, true );
      if( file != null ) {
	if( this.readOnly ) {
	  /*
  ...
  {
    String fName = nextArgFileName( false, true );
    synchronized( this.lockObj ) {
      checkDisk();
      if( this.raf != null ) {
	throwFileOpen();
  ...
  private void execDir() throws VdipException
  {
    ...
    if( (this.cmdArgPos >= 2) && (this.cmdArgPos < this.cmdLineLen) ) {
      fName = nextArgFileName( true, true );
      if( fName == null ) {
  ...
  private void execDirIO() throws VdipException
  {
	...
	if( files != null ) {
	  ...
	  /*
	   * Auf einem Betriebssystem mit Unterscheidung zwischen
	   * Gross-/Kleinschreibung kann nach der Wandlung des Dateinamens
	   * in Grossbuchstaben der gleiche Dateiname mehrfach vorkommen.
	   * Eine Mehrfachausgabe dieser Dateien wird unterbunden.
	   */
	  Set<String>        listedNames = new TreeSet<>();
	  VFATFileNameMapper fnMapper    = this.fileNameMapper;
	  for( File f : files ) {
		...
		if( this.fileNameMapperEnabled ) {
		  String sName = DeviceIO.getShortPathName( f.getPath() );
		  if( sName != null ) {
		    sName = (new File( sName )).getName();
		  }
		  if( sName != null ) {
		    if( VFATFileNameMapper.isValidShortFileName( sName ) ) {
		      fName   = sName;
		      isShort = true;
		    }
		  }
		  if( !isShort && (fnMapper != null) ) {
		    fName   = fnMapper.toShortName( fName, curDir );
		    isShort = true;
		  }
		}
		if( !isShort ) {
		  if( !VFATFileNameMapper.isValidShortFileName( fName ) ) {
		    fName = null;
		  }
		}
		if( fName != null ) {
		  if( !listedNames.contains( fName ) ) {
		    listedNames.add( fName );
  ...
  {
    String fName = nextArgFileName( true, true );
    synchronized( this.lockObj ) {
      putResultByte( '\r' );	// Leerzeile, auch bei Fehlermeldung
      checkDisk();
      File file = findFile( fName, true );
	...
	throwCommandFailed();
      ...
      Long          lastModiefiedMillis  = ftd.getLastModifiedMillis();
      if( lastModiefiedMillis != null ) {
	lastModifiedDateTime = getDateTimeByMillis(
					lastModiefiedMillis.longValue() );
	/*
	...
	lastAccessDateTime = lastModifiedDateTime;
      ...
      Long creationMillis = ftd.getCreationMillis();
      if( creationMillis != null ) {
	creationDateTime = getDateTimeByMillis(
					creationMillis.longValue() );
      }
      Long lastAccessMillis = ftd.getLastAccessMillis();
      if( lastAccessMillis != null ) {
	lastAccessDateTime = getDateTimeByMillis( 
					lastAccessMillis.longValue() );
      }
      putString( fName );
      putResultByte( '\u0020' );
  ...
  {
    String dirName = nextArgFileName( false, false );
  ...
  private void execOpenFileForWrite() throws
					...
					VdipException
  {
    String fName  = nextArgFileName( false, false );
    Long   millis = nextOptArgDatetimeInMillis();
    synchronized( this.lockObj ) {
      checkDisk();
      if( (this.raf != null) && this.fileWrite ) {
  ...
  {
    String srcName = nextArgFileName( false, true );
    String dstName = nextArgFileName( false, false );
    synchronized( this.lockObj ) {
      checkDisk();
	...
	throwCommandFailed();
      ...
      /*
       * Wenn fuer die Zieldatei ein langer Dateiname existiert,
       * dann ist Umbenennen nicht moeglich
       */
      VFATFileNameMapper fnMapper = this.fileNameMapper;
      if( fnMapper != null ) {
	if( fnMapper.hasLongName( dstName ) ) {
	  throwCommandFailed();
	}
      }
      ...
      // Argumente auswerten
      File curDir  = this.curDir;
      File srcFile = findFile( srcName, true );
      File dstFile = findFile( dstName, false );
  ...
  private File findFile(
			String  fileName,
			boolean ignoreCase ) throws VdipException
  {
    File file    = null;
    File curDir  = this.curDir;
    File rootDir = this.rootDir;
  ...
  {
    long rv = DEFAULT_DATETIME_VALUE;
    if( millis != null ) {
      rv = 0;
      synchronized( this.cal ) {
	this.cal.clear();
	this.cal.setTimeInMillis( millis );
	int year = this.cal.get( Calendar.YEAR ) - 1980;
	if( year > 0 ) {
	  rv |= ((year << 25) & 0xFE000000L);
	}
	rv |= (((this.cal.get( Calendar.MONTH ) + 1) << 21) & 0x01E00000L);
	rv |= ((this.cal.get( Calendar.DAY_OF_MONTH ) << 16) & 0x001F0000L);
	rv |= ((this.cal.get( Calendar.HOUR_OF_DAY ) << 11) & 0x0000F800L);
	rv |= ((this.cal.get( Calendar.MINUTE ) << 5) & 0x000007E0L);
	rv |= ((this.cal.get( Calendar.SECOND ) / 2) & 0x0000001FL);
      }
    }
    return rv;
  }
  ...
  private long getMillisByDateTime( long value )
  {
    long millis = 0;
    synchronized( this.cal ) {
      this.cal.clear();
      this.cal.set(
		...
		(int) (value >> 5) & 0x3F,
		(int) (value & 0x1F) * 2 );
      millis = this.cal.getTimeInMillis();
    }
    return millis;
  }
  ...
  private String nextArgFileName(
			boolean allowDotDirs,
			boolean enableLongName ) throws VdipException
  {
    String rv = null;
    if( this.cmdArgPos >= 2 ) {
	...
	if( basePos > 0 ) {
	  ...
	  for( int i = 0; i < basePos; i++ ) {
	    if( !VFATFileNameMapper.isValidShortFileNameChar(
						baseBuf[ i ] ) )
	    {
	      throwFilenameInvalid();
	    }
	  ...
	  if( extPos > 0 ) {
	    ...
	    for( int i = 0; i < extPos; i++ ) {
	      if( basePos < baseBuf.length ) {
		char ch = extBuf[ i ];
		if( VFATFileNameMapper.isValidShortFileNameChar( ch ) ) {
		  baseBuf[ basePos++ ] = ch;
		} else {
		  throwFilenameInvalid();
		}
	  ...
	  rv = String.valueOf( baseBuf, 0, basePos );

	  // ggf. langen Dateiname zurueckliefern
	  if( enableLongName ) {
	    VFATFileNameMapper fnMapper = this.fileNameMapper;
	    if( fnMapper != null ) {
	      rv = fnMapper.getLongByShort( rv );
	    }
	  }
	} else {
	  throwFilenameInvalid();
   ...
   * Ist ein ungueltiger Zeitstempel angegeben,
  ...
  private Long nextOptArgDatetimeInMillis() throws IncompleteCmdException
  {
	...
	int  d = (int) ((v >> 16) & 0x1F);	// Tag
	int  m = (int) ((v >> 21) & 0x0F);	// Monat
	if( (d >= 1) && (d <= 31) && (m >= 1) && (m <= 12) ) {
	  millis = getMillisByDateTime( v );
	}
      }
      catch( VdipException ex ) {}
    }
  ...
  private void putResultByte( int b )
  {
    boolean empty = false;
    synchronized( this.resultQueue ) {
      empty = this.resultQueue.isEmpty();
  ...
  private void updLastModified( boolean forceUpd )
  {
    if( this.fileWrite && !this.forceCurTimestamp) {
      File file = this.file;
      if( file != null ) {
	long curMillis = System.currentTimeMillis();
	if( forceUpd || (this.lastUpdFileMillis < (curMillis - 1000)) ) {
	  Long millis = this.fileMillis;
	  if( millis == null ) {
	    if( this.defaultFileMillis == null ) {
	      this.defaultFileMillis = Long.valueOf(
			getMillisByDateTime( DEFAULT_DATETIME_VALUE ) );
	    }
	    millis = this.defaultFileMillis;
	  }
	  if( (file != null) && (millis != null) ) {
	    file.setLastModified( millis.longValue() );
	    this.lastUpdFileMillis = curMillis;
	  }
	}
      }
    }
