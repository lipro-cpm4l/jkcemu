...
public class HexCharFld extends JComponent
{
  ...
  public void setDataBytes( byte[] dataBytes, int dataLen )
  {
    this.dataBytes = dataBytes;
    this.dataLen   = Math.min( dataLen, dataBytes.length );
    this.totalRows = (this.dataLen + BYTES_PER_ROW - 1) / BYTES_PER_ROW;
    this.topRow    = 0;
    this.caretPos  = -1;
    this.markPos   = -1;
    invalidate();
  ...
  public void paintComponent( Graphics g )
  {
	...
	m2 = this.caretPos;
      ...
      if( (this.dataBytes != null) && (pos < this.dataLen) ) {
	while( (pos < this.dataLen) && (y < h + hFont) ) {
	  g.setColor( SystemColor.textText );
	  g.drawString( String.format( "%05X", pos ), MARGIN, y );
	  int x = this.xHex;
	  for( int i = 0; i < BYTES_PER_ROW; i++ ) {
		...
		g.setColor( SystemColor.textText );
	      ...
	      g.drawString(
			String.format( "%02X", this.dataBytes[ idx ] ),
			x,
			y );
	      x += this.wHex;
	    } else {
	  ...
	  for( int i = 0; i < BYTES_PER_ROW; i++ ) {
	    int idx = pos + i;
	    if( idx < this.dataLen ) {
	      char ch = (char) ((int) this.dataBytes[ pos + i ] & 0xFF);
	      if( (ch < 0x20) || (ch > 0x7F) ) {
		ch = '.';
	      }
	      if( (idx >= m1) && (idx <= m2) ) {
	...
	g.setColor( SystemColor.textText );
	g.drawString( "Datei ist leer", MARGIN, y );
      }
      if( this.xOffset > 0 ) {
	g.translate( this.xOffset, 0 );
      }
